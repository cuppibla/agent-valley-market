...
def main() -> None:
    ...
    # list means chapter 4's edit is in; SqliteSessionService means chapter 3's is.
    try:
        ...
        print(f"{OK} street     · workflow · {len(names)} nodes"
              f"{'  · the manager is in' if gate else '  · no manager yet (chapter 4 adds one)'}")
        import street.service as svc
        store = type(svc._sessions).__name__
        print(f"{OK} ledger     · {store}"
              f"{'' if store.startswith('Sqlite') else '  · in memory (chapter 3 moves it to a file)'}")
